...
const ProtectedCourseRoute = () => {
  ...
  useEffect(()=> {
    let checkCourse = userCourseService.checkCourse(id).then(() => {
      checkCourseEntry.current = true;
    }).catch(err => {
      checkCourseEntry.current = false;
    })

    let checkErrorCourse = userCourseService.checkEnrollCourse(id).then(() => {
  ...
  const loaded = (checkCourseEntry.current && !loading)? <CourseContent /> : <CourseEnroll checkEnroll={checkEnroll.current} />

  return loading? renderLoader : loaded;
};

export { ProtectedRoute, ProtectedInstructorRoute, ProtectedUserRoute, ProtectedCourseRoute };
